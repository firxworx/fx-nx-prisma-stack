...
export function useVideosQuery(pctx: ParentContext): UseQueryResult<VideoDto[]> {
  return useQuery<VideoDto[]>(cacheKeys.list.all(), () => fetchVideos({ parentContext: pctx?.parentContext }), {
...
export function useVideoQuery({
  parentContext,
  uuid,
}: ParentContext & { uuid: string | undefined }): UseQueryResult<VideoDto> {
  return useQuery<VideoDto>(cacheKeys.detail.unique(uuid), () => fetchVideo({ parentContext: parentContext, uuid }), {
...
): UseMutationResult<VideoDto, Error, CreateVideoDto & ParentContext> {
  const queryClient = useQueryClient()

  return useMutation<VideoDto, Error, CreateVideoDto & ParentContext>(fetchCreateVideo, {
...
): UseMutationResult<VideoDto, Error, ApiMutateRequestDto<UpdateVideoDto> & ParentContext> {
  const queryClient = useQueryClient()

  return useMutation<VideoDto, Error, ApiMutateRequestDto<UpdateVideoDto> & ParentContext>(fetchMutateVideo, {
...
export function useVideoDeleteQuery(
  ...
  return useMutation<void, Error, ApiDeleteRequestDto & ParentContext, VideoDeleteQueryContext>(fetchDeleteVideo, {
    onSuccess: async (data, vars, context) => {
