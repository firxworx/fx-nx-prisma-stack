...
/**
 * Header logo that links to home ('/') route.
 */
const LogoLink: React.FC<{ appendClassName?: string }> = ({ appendClassName }) => {
  return (
    <Link href="/">
...
> = ({ navigationLinks, linkClassName, linkCurrentClassName, onLinkClick }) => {
  ...
  // check is tolerant if there is no trailing slash on router.pathname
  const isCurrentMenuLink = (routerPathName: string, itemHref: string): boolean => {
    return itemHref.includes(routerPathName)
  }
...
const DesktopNavMenu: React.FC<Pick<HeaderProps, 'navigationLinks'>> = ({ navigationLinks }) => {
  return (
    <div className="hidden space-x-5 lg:flex lg:justify-start lg:items-center lg:mx-10 lg:flex-1 text-slate-900">
      <MenuLinks
        ...
        <div className="-mr-2">
          <Popover.Button
            className={clsx(
              ...
              'hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-slate-100',
            )}
          >
    ...
    <Popover as="header" className={clsx('relative border-b-2 bg-slate-100 border-slate-200')}>
      {({ open, close }) => (
        <>
          <nav
            className={clsx(
              'relative mx-auto flex items-center justify-between py-4',
            ...
            <div className="flex items-center flex-1">
              <div className="flex items-center justify-between w-full lg:w-auto">
                <div className="flex items-center space-x-4">
                  <LogoLink appendClassName="-mt-2" />
                  <div className="border-r border-r-gray-500 h-full w-1" />
                </div>
                <div className="flex items-center lg:hidden">
                  <Popover.Button
                    className={clsx(
                      'inline-flex items-center justify-center p-2 rounded-md',
                      'text-gray-400 bg-white border-2 border-slate-200 hover:bg-gray-100',
